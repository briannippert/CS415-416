 ...
 * that control the Hiker's path and affec
 * t the nGoals including the emergency
 * nGoals.
 * 
 * @author rdb 01/27/15
 */
...
public class HikerPanel extends JPanel implements Animated, MouseListener
{
    ...
    private Point[] makePath( int n )
    {
        ...
        for ( int i = 0; i < n; i++ )
        {
            int x2 = ranGen.nextInt( xMax );
            int y2 = ranGen.nextInt( yMax );
            array[i] = new Point( x2, y2 );
            Point tempp = new Point( x2, y2 );
            _s1.push( tempp );
        }
        return array;
    }
    ...
    public void newFrame()
    {
        ...
        if ( canpop == true )
        {
            ...
            float c = (float) ( (float) ( Math.pow( xdist, 2 ) ) + ( Math
                    .pow( ydist, 2 ) ) );

            dist = (float) Math.sqrt( c );
            steps = dist / speed;
            dx = xdist / steps;
            dy = ydist / steps;
        ...
        {
            _h1.setLocation( (int) dx + _h1.getLocation().x,
                    (int) dy + _h1.getLocation().y );
            x++;
