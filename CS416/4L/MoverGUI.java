...
public class MoverGUI extends JPanel implements NewFrame 
{
  ...
  public MoverGUI() 
  {
    ...
    // 2a. change setLayout to "new BorderLayout()".
  ...
  private void handleButtonEvent( int buttonId )
  {
    ...
    else if(buttonId == 2)
    {
      ...
      float dx = _snowman.getMoveX();
      float dy = _snowman.getMoveY();
      float fdx = dx * fasterRatio+1 ;
      float fdy = dy * fasterRatio+1 ;
      int movex = (int)fdx;
      int movey = (int)fdy;
          _snowman.setMove(movex +1,movey +1);
    ...
    else if(buttonId == 3)
    {
      ...
      float dx = _snowman.getMoveX();
      float dy = _snowman.getMoveY();
      float fdx = dx * slowerRatio;
      float fdy = dy * slowerRatio;
      int movex = (int)fdx;
      int movey = (int)fdy;
      if(minStepFlag == false)
        
          _snowman.setMove(movex,movey);
  ...
  private JToggleButton makeStepSizeToggle()
  {
    ...
    ///////////////////////////////////////////////////////////
    JToggleButton toggle = new JToggleButton( "Enable Min Step Size" );
      ...
      {  
        ...
        if(minStepFlag == true)
          minStepFlag = false;
            ...
            minStepFlag = true;
        System.out.println( minStepFlag ); 
      }
    });
    return toggle;
  }
  ...
  public static void readArgs( String[] args )
  {
    ...
    //             MoverGUI.dxDefault (or just dxDefault)
    //     if args[ 1 ] not present, or not an int, use
    //             MoverGUI.dyDefault (or just dyDefault)
    ...
    if(args.length > 0)
    {
      try
      {
        MoverGUI.dxInitialMove = Integer.parseInt( args[0] );
        MoverGUI.dyInitialMove = Integer.parseInt( args[1] );
      }
      catch(Exception e)
      {
