...
public class Q3_orderNeighborsList
{
    ...
    public static class List
    {
        ...
        // -------------------- orderNeighbors( Node ) -----------------------
        /**
         * Traverse the list once recursively. For every adjacent pair of
         * nodes, check if they are in order. i.e., the first of the pair
         * should have a "value" field that is LESS than the 2nd of the pair.
         * 
         * If they are not in order, swap the two NODES, NOT THEIR VALUES I.e.,
         * change the links in the list so that the 2 nodes are reordered.
         */
        ...
        public void check()
        {
            ...
            while ( cur != tail && count < size )
            {
                if ( cur != cur.next.prev )
                    System.out
                            .println( "***********Inconsistent links from: "
                                    + cur + " to " + cur.next );
                if ( cur.prev.next != cur )
                    System.out
                            .println( "************Inconsistent link from: "
                                    + cur.prev + " to " + cur );
                cur = cur.next;
                count++;
            }
