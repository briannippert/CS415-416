...
public class FractalGUI extends JPanel 
{
    ...
    private  double   defaultHWratio  = FractalEllipse.hwRatio; 
    ...
    public FractalGUI( int w, int h )     
    {
        ...
        { 
            /** Anonymous class overrides paintComponent.
    ...
    private void reset()
    {
        ...
        FractalEllipse.hwRatio   = defaultHWratio;
        
        _sizeSlider.setValue( (int) Math.round( defaultSizeRatio * 100 ) );
        _offsetSlider.setValue( (int) Math.round( defaultOffset * 100 ) );
        _widthSlider.setValue( defaultWidth );
        _hwRatioSlider.setValue( (int) Math.round( defaultHWratio * 100 ) );
     ...
     * the offset of the position of a child ellipse on its parent's edge.
     * The integer percentage value from the slider must be converted to a
     * floating point (double) fraction. This value is assigned to the 
    ...
    /**
     * responds to a slider that sets the height for the initial ellipse. 
    ...
    public class DepthListener implements ChangeListener
    {
        //------------------- stateChanged -----------------------------
        /**
         * Invoked whenever user changes the state of spinner.
         * @param ev ChangeEvent
         */
        public void stateChanged( ChangeEvent ev )
        {
     ...
     * to fill the ellipses or just draw outlines.
     */
    ...
    /**
     * Responds to an event from a JRadioButton that identifies whether to fill
     * the triangles with opaque colors; otherwise uses semi-transparent.
     */
    private class SolidFillListener implements ActionListener
    {
        ...
        public void actionPerformed( ActionEvent ev )
        {
            FractalEllipse.solidFill = !FractalEllipse.solidFill;
            makeScene();
        }
    }
