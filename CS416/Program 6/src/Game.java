...
public class Game extends JPanel implements NewFrame
{
    ...
    // ----------------------- instance variables
    // -------------------------------
    public static Pile _drawPile = null;
    public static Pile[] _play; // play piles
    public static Pile _discards = null;
    ...
    // -------------------------------------
    /**
     * Starts the recursive call to build the tree. Creates root node (since it
     * is special) then lets recursion take over.
     */
    public void buildTree()
    {
        ...
        {
            TreeNode n1 = new TreeNode( parent, treeDepth );
            GameState g1 = new GameState();
            n1.setGameState( g1 );
            ArrayList<Integer> a1 = g1.findMoves();
            n1.setMoves( a1 );
            parent = n1;
            ...
            for ( Integer i : a1 )
            {
                ...
                // curGS = gs // restore state of parent
                // make the move (changes curGS)
                Move m = new Move( i );
                TreeNode child = new TreeNode( n1, treeDepth + 1 );
                nodeCount++;
                child.child( m );
                max++;
                buildTree();
        ...
        // TreeNode makeNode = new TreeNode(parent, m1,treeDepth)
        // //////////////////////////////////////////////////
    ...
    // ----------------------------------------------------
    /**
     * Undo a move. Only works if the tree is built; Returns a string if a
     * problem occurs "Undoes" the move by setting the game state.
     * 
     * @return String
     */
    public String undo()
     ...
     * 
     * @param moveFromId
     *            int 0 => drawPile, 1-4 => play Piles 5 is discard; illegal
     *            move by definition
     * 
     * @return boolean ltest
     */
    private boolean cardPicked( int moveFromId )
    {
        if ( root != null )
        {
        ...
        // System.out.println( "Card Picked Called" );
        m1 = new Move( moveFromId );
        m1.executeMove();
        this.update();
        this.updateLabels( null );
        System.out.println( "Move ID: " + moveFromId );
        if ( m1.hasMoved() == true )
        {
            this.update();
            return true;
     ...
     * AutoMove mode. If a tree is not built, it needs to invoke code to find a
     * valid move from the current state. If a tree is built, it chooses the
     * best move from the current state information in the tree.
     * 
     * @return String null: there was a move and it has been done. non-null: a
     *         message informing that no move found
     */
    private String makeAMove()
    {
        String msg = null;

        if ( this.cardPicked( 1 ) == true )
        {
            System.out.println( "Card Picked 1" );
            numberOfMoves++;
        }
        else if ( this.cardPicked( 2 ) == true )
        {
            System.out.println( "Card Picked 2" );
            numberOfMoves++;
        }
        else if ( this.cardPicked( 3 ) == true )
        {
            System.out.println( "Card Picked 3" );
            numberOfMoves++;
        }
        else if ( this.cardPicked( 4 ) == true )
        {
            System.out.println( "Card Picked 4" );
            numberOfMoves++;
        }
        else
        {
            if ( _drawPile.getStack().isEmpty() == true )
            {
            ...
            {
                System.out.println( "Draw Cards Called" );
    ...
    /**
     * Update the labels for potential moves with the subtree "score" for all
     * next possible moves.
     * 
     * @param cur
     *            TreeNode
    ...
    /**
     * Gets the heighest card in the play piles.
     * 
     * @return int int
     */
    public static int getHeighest()
    {
        for ( int i = 0; i < 4; i++ )
        {
            if ( _play[i].getStack() != null
    ...
    /**
     * Returns the DrawPile for the gamestate.
     * 
     * @return play
     */
    public static Stack<Card> getDrawPile()
    {
        return _drawPile.getStack();
    }

    /**
     * Gets the play piles for the GameState.
     * 
     * @return play
     * 
     */

    public static Stack<Card>[] getPlayPiles()
    {
        @SuppressWarnings( "unchecked" )
        Stack<Card>[] stack = (Stack<Card>[]) new Stack[4];
        for ( int i = 0; i < 4; i++ )
        {
            stack[i] = _play[i].getStack();
        }
        return stack;
    }

    /**
     * gets the discard piles.
     * 
     * @return discards
     */
    public static Stack<Card> getDiscard()
    {
        return _discards.getStack();
    }
     ...
     * @return discards
     */
    public static Pile getDiscardPile()
    {
        return _discards;
    }
     ...
     * @return play pile
     */
    public static Pile[] getPlayPile()
    {
        return _play;
    }
     ...
     * @return draw pile
     */
    public static Pile getDrawPiles()
    {
        return _drawPile;
    }
    ...
    // ------------------------ replay() -----------------------------------
    /**
     * Starts the game over, including deleting the tree. Resets all the piles
    ...
    // --------------------------------------
    /**
     * Shuffles the current base deck according to the seed in the game. This
     * version leads to more predictable games. Each seed is independent and
     * should generate same series of results regardless of the order in which
     * the seeds are used.
     */
    public void newGame()
    {
     ...
     * will exit AutoPlay. Returns a string with no-null value if no moves
     * possible.
     * 
     * @return String
     */
    ...
    // ----------------------------------------------------
    /**
     * Get a string representation of the path from current tree node to the
     * best end state. Log that string to Reporter.log.
     * 
     * @return String
     * @param prefix
     *            String
    ...
    // -------------- nodeStats() ------------------------------------
    /**
     * Create a String providing basic statistics about the subtree rooted at
     * the current tree Node. Send to Reporter.log.
     * 
     * @return String prefix String null if success, error message if not.
     * @param prefix
     *            String
     */
    ...
    // ------------------------ initializePiles() ---------------------------
    /**
     * Intializes the piles at the start of the program. Create them and set
     * their locations.
     */
