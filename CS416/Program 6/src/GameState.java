 ...
 * 
 * This is a real skeleton containing only a couple of signatures needed for
 * producing consistent output for testing.
 * 
 * A major decision on your part is between: a. Actually copying state
 * variables: the 6 card piles. b. Defining an incremental state definition:
 * Given the starting piles, just save state change information -- the Move
 * made.
 * 
 * @author Brian Local
 */
...
public class GameState
{
    ...
    public GameState ( )
    {
        _drawPile = Game.getDrawPile();
        _piles = Game.getPlayPiles();
        _discardPile = Game.getDiscard();
    }
     ...
     * @return piles
     */
    public Stack<Card>[] getPlayPiles()
    {
        return _piles;
    }
    ...
    {
        return "Piles[" + _drawSize + " " + _pileSizes[0] + " "
                + _pileSizes[1] + " " + _pileSizes[2] + " " + _pileSizes[3]
                + " " + _discardSize + "]";
    }
}