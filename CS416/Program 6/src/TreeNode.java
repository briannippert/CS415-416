...
/**
 * TreeNode class for Aces Up. The data is the current game state Node keeps
 * track of its children, its parent, the move that caused the game to get to
 * this state and the best move that leads to the best game state
 * 
 * @author jb Created 7/13/10 for general use
 * 
...
public class TreeNode
{
     ...
     *            depth
     */
    public TreeNode ( TreeNode p, int d )
    {
    ...
    /**
     * Finds the availible moves.
     * 
     * @param a1
     *            integer
     */
    public void setMoves( ArrayList<Integer> a1 )
    {
        moves = a1;
    }
    ...
    public String getDrawString()
    {
        ...
        for ( int i = 1; i < 5; i++ )
        {
            cardStr += Game._play[i].getStack().peek().toString();
        }
    ...
    // ---------------- getPathString() ------------------------------------
    /**
     * Returns a String representing a path from this node to the best leaf
     * node. It should be a space separated list of the 2-char id's generated
     * by the Card.toString() method.
     * 
     * @return String.
     */
    ...
    // --------------- toString( String ) ------------------
    /**
     * Recursive method that returns a string of all of the children Each
     * treenode is represented by the card moved to get to its point If it was
     * a draw, it says draw instead If there is only one child, it will display
     * itself on the same line If there is more than one child, it indents and
     * displays the gamestate then indents again and acts like there was only
     * one child.
     * 
     * @param indent
     *            String
