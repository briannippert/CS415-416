...
public class BiNode
{
  ...
  private void updateData()
  {
    ...
    {
      this.data.count = this._left.size() + this._right.size();
    }
    else
      this.data.count = this.size();
  ...
  {        
    ...
    if(this.intersects(min,max))
    {
      for(int i = 0; i <_values.size(); i++)
      {
        if(_values.get(i) <= max && _values.get(i) >= min)
          found.add(_values.get(i));
      }
    }
    if(this._left != null && _left.intersects(min,max))
      this._left.findPoints(min,max,found);
    
    if(this._right != null && _left.intersects(min,max))
      this._right.findPoints(min,max,found); 
    for ( int i = 0; i < _values.size(); i++ )
    {
      System.out.print( _values.get( i ) + " " );
    }
    System.out.println();
