 ...
 * 
 * 03/04/13 rdb
 *       Changed interface, so user can enter multiple words for testing
 *           at one time. 
 */
import javax.swing.*;
...
public class Anagram //extends JFrame
{
    ...
    private static String    _dsType;     // search type incl table size
    private static int       _tableSize = 53;   // size for our HashTable
    private static char      _hashFunction = 'x';  // code for bad hash 
    private static Scanner   _batchIn = null;
    private static String lastWords = "";  // simple history so can keep 
    // looking up same words over 
    // and over again
    ...
    {      
        ...
        for ( String word: words )
        {
            ...
            {   
                ...
                lookup( validWords, "", word );
                
                float secs = ( System.currentTimeMillis() - start ) / 1000.0f;
                
                
                outMessage += makeMessage( word, validWords, secs );
                outMessage += "\n----------------------------------------------\n";
                outHeader += word + "\t";
                outTimes  += secs + "\t";
    ...
    private String readWords()
    {
        ...
        if ( _batchIn != null )
        {
            if ( _batchIn.hasNextLine() )
                words = _batchIn.nextLine();
            else
                words = null;
        }
        else  // interactive mode
        {
            String inMessage = "Enter words to be found";
            words = JOptionPane.showInputDialog( null,  inMessage );
        }
        
        if ( words != null && words.length()  > 0 ) // non-empty input
        {
            if ( words.trim().equals( "." ) )   // if it's . use last input
                words = lastWords;
            else
                lastWords = words;               // else save as last input
        }
                         ...
                         String head, String tail )
    {
        ...
        if ( tail.length() == 0 ) // no tail => have possible word
        {
    ...
    private String makeMessage( String start, Collection<String> validWords, 
                               float time ) 
    {
        String message = start + ": makes " + validWords.size() 
                   + " words; using " + _dsType ;
        int i = 0;
    ...
    {  
        ...
        {
            System.err.println( "No data structure specified! Using HashSet!" );
            option = "h";
        }
        _dsName = option;
        ...
        switch ( _option )
        {
            ...
            case 'j':   // java's hash function with our hash table
                _dictionary = 
                      new HashTable<String>( _tableSize, _option, option );
                _dsType = "Hash:" + _dsName + "-" + _tableSize;
                break;
        }
        readDictionary( _dictionary );
        if ( _dictionary instanceof HashTable )
    ...
    public static void report()
    {
        if ( _dictionary instanceof HashTable )
            ...
            System.out.println( "Current dictionary is not a HashTable" );
    }
     ...
     * the chosen option. The returned string is just a term referring to
     * chosen algorithm.
     */
